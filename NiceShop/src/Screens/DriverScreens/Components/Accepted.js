...
import { ScrollView, Box, HStack, Icon, Flex, Center, Pressable, VStack, Stack, Heading, Button } from 'native-base';
...
const Accepted = () => {
    ...
    const acceptedDeliveries = deliverydata.filter((delivery) => delivery.status === "Accepted");

    return (
      <ScrollView bg={Colors.newgray} pb={10}  style={{ height: 570 }} showsVerticalScrollIndicator={false} >
          {acceptedDeliveries.map((delivery, index) => (
              <Center 
                  key={index}
                  bg={Colors.white} 
                  mt={3} 
